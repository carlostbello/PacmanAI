...
class CornersProblem(search.SearchProblem):
    ...
    def getStartState(self):
        ...
        start_state = (self.startingPosition, [
                       False for corner in self.corners])
        return start_state
    ...
    def getSuccessors(self, state):
        ...
        for action in [Directions.NORTH, Directions.SOUTH, Directions.EAST, Directions.WEST]:
            ...
            corners_reached = corners_reached.copy()
            x, y = pos
            dx, dy = Actions.directionToVector(action)
            nextx, nexty = int(x + dx), int(y + dy)
            ...
            if not self.walls[nextx][nexty]:
                if next_pos in self.corners:
                    if not corners_reached[self.corners.index(next_pos)]:
                        corners_reached[self.corners.index(next_pos)] = True
                nextState = (next_pos, corners_reached)
...
def cornersHeuristic(state, problem):
    ...
    corners_dist = []
    pos, corners_reached = state
    for index, corner in enumerate(corners_reached):
        if not corner:
